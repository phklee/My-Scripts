...
XAVIER_LOG_PATH=/media/nvidia/idriverplus/record_bags/bags
...
function CheckDocumentPath(){
	...
	echo "$1"
	if ! sshpass -p ${XAVIER_PASSWORD} ssh ${remove_yes} ${REMOTE_COMPUTER_NAME}@$1 '[ -e '${g_document_path}' ]'
	then
...
function CopyFiles(){
    ...
    for i in `sshpass -p ${XAVIER_PASSWORD} ssh ${remove_yes} ${REMOTE_COMPUTER_NAME}@$1 ' ls '${g_document_path}' ' `
    do
       GetFileUtc $i
       if [ ${g_file_utc} -le ${g_user_higher_utc} ] && [ ${g_file_utc} -ge ${g_user_lower_utc} ]
       then
